...
/* AFTER */
--light-text-primary: oklch(0.35 0.04 160);    /* dark forest green — primary body text */
--light-text-secondary: oklch(0.50 0.03 160);  /* medium forest green — secondary text */
```
...
/* AFTER */
--color-heading: var(--color-tide);  /* tide = oklch(0.49 0.08 205.88) — deep teal-green */
```
...
```css
/* BEFORE */
--light-primary-accent: oklch(0.7732 0.0906 125.78);  /* sage green #a7c080 — mismatched */
...
/* AFTER */
--light-primary-accent: var(--color-avocado-500);  /* oklch(0.84 0.18 117.33) — vibrant green */
--light-secondary-accent: var(--color-coral);       /* keep — warm bridge accent */
--light-highlight: var(--color-dusk);                /* golden highlight — nods to dark theme */
...
/* AFTER */
--color-accent-glow: oklch(0.70 0.12 160);  /* forest green glow for hero */
```
...
--color-md-glow: oklch(0.55 0.10 170 / 80%);        /* mid-depth forest */
--color-outer-glow: oklch(0.30 0.06 180 / 30%);     /* deep forest shadow */
...
--dark-text-primary: oklch(0.90 0.005 90);        /* warm near-white */
...
3. **Add transition utility classes** after the keyframes:
...
4. **In `background-effects.css`**: Remove the keyframe definitions (they now live in `motion.css`), but keep the class definitions (`.grain-overlay`, `.glass-overlay`, `.water-droplets`, `.forest-texture`, `.amber-glow`, `.mist-layer`, `.vignette`, `.scanlines`, `.steam-overlay`) that reference those keyframes.

5. **In `utilities.css`**: Remove the `blink` keyframe definition (now in `motion.css`). Keep the `.cursor-blink` class that references it.
...
--color-scanline: color-mix(in srgb, var(--color-surface-base), transparent 90%);
```
